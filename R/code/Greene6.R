...
y <- (w + epsilon > 0) + (w + epsilon > 1) + (w + epsilon > 2) # change this for different (number of) limits

# ordered probit
beta <- rep(0,k)
tau <- c(0.9,2.2) # make sure initialisation is separated enough # change this for different (number of) limits
p <- length(tau)
gamma <- c(beta,tau)
alpha <- rep(0,N)
...
while(sum(dif > eps)>0){
  ...
  step <- steplength(0.9,0.1,1,c(gamma,alpha),x,c(deltagamma,deltaalpha),c(ggamma,galpha))
  stepold <- cbind(stepold,step)
