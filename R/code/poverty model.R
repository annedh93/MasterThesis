...
#samp <- data
#samp <- routing(samp)
#save(samp,file="C:\\Users\\anned\\OneDrive\\Documenten\\master thesis\\R\\data\\samp.Rdata")
...
rsamp <- samp[-which(is.na(samp[,"sphus"])),]
IADLi <- rsamp[,"IADL"]>0
...
c <- which(colnames(rsamp)%in%c("numberid","id","wave","country","firstwave","age","mstat","isced",
                                "child","casp","sphus","IADL","IADLi","employment","lifesat"))
...
for(m in 1:M){
  ...
  #library(plm)
  panel <- as.data.frame(cbind(imp[,c("numberid","id","wave","sphus","lifesat","IADL","IADLi","child")],
  ...
  formula <- as.formula(paste(paste("lifesat ~"),paste(regressors,collapse="+")))
