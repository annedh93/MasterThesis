...
beta <- c(1,2) # change this for different number of parameters
o <- length(beta)
...
y <- (w + epsilon > 0) + (w + epsilon > 1.5) + (w + epsilon > 3) # change this for different (number of) limits

# ordered probit
k <- 2*o
x <- eval(parse(text = paste0("array(c(x,",paste0("matrix(rowMeans(x[,,",1:o,"]),N,T,byrow=FALSE)",collapse=","),"),dim=c(N,T,k))")))
beta <- rep(0,k)
tau <- c(1.5,2.5) # make sure initialisation is separated enough # change this for different (number of) limits
p <- length(tau)
gamma <- c(beta,tau)
...
while(sum(dif > eps)>0){
  ...
  step <- steplength(0.9,0.1,1,gamma,x,deltagamma)
  show(step)
  gammaold <- cbind(gammaold,gamma)
  gamma <- gamma + step*deltagamma
...
steplength <- function(ro,c,abeg,gamma,x,y,deltagamma){
  step <- abeg
  while(LL(gamma + step*deltagamma,x) <= (LL(gamma,x) + c*step*t(ggamma)%*%deltagamma)){
    step <- ro*step
  }
  return(step)
}
